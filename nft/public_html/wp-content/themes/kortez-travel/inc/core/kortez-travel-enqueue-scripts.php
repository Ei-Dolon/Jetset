...
if( !function_exists( 'kortez_travel_scripts' ) ){
	function kortez_travel_scripts() {
		...
		wp_enqueue_style( 'kortez-travel-blocks', get_template_directory_uri() . '/assets/css/blocks.min.css' );
		wp_enqueue_style( 'kortez-travel-style', get_stylesheet_uri() );
		wp_enqueue_style( 'kortez-travel-google-font', wptt_get_webfont_url( 'https://fonts.googleapis.com/css?family=Open+Sans:300,400,400i,600,700,800|Poppins:300,400,400i,500,600,700,800,900&display=swap' ), false );
		...
		$scripts = array(
			...
			array(
				'id'     => 'kortez-travel-skip-link-focus-fix',
				'url'    => get_template_directory_uri() . '/assets/js/skip-link-focus-fix.js',
				'footer' => true
			),
			array(
				'id'     => 'kortez-travel-navigation',
			...
			array(
				'id'     => 'kortez-travel-custom',
				'url'    => get_template_directory_uri() . '/assets/js/custom.min.js',
				'footer' => true
			)
		);

		kortez_travel_add_scripts( $scripts );
		...
		$locale = array(
			...
			'the_custom_logo'            => kortez_travel_get_custom_logo_url(),
		);
		$locale = apply_filters( 'kortez_travel_localize_var', $locale );
		wp_localize_script( 'kortez-travel-custom', 'KORTEZTRAVEL', $locale );
...
add_action( 'wp_enqueue_scripts', 'kortez_travel_scripts' );
...
* @since Kortez Travel 1.0.0
*/
function kortez_travel_add_scripts( $scripts ){
...
* @since Kortez Travel 1.0.0
*/
function kortez_travel_block_editor_styles() {
	// Block styles.
	wp_enqueue_style( 'kortez-travel-block-editor-style', get_theme_file_uri( '/assets/css/editor-blocks.min.css' ) );
	// Google Font
	wp_enqueue_style( 'kortez-travel-google-font', 'https://fonts.googleapis.com/css?family=Poppins:300,400,400i,500,600,700,700i', false );
}
add_action( 'enqueue_block_editor_assets', 'kortez_travel_block_editor_styles' );